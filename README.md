...
- The backend application utilizes a **History-Aware Retriever Chain**. This analyzes the user query *and* chat history context, retrieves the Top-K (`k=10`) most matching FAISS chunks, and funnels this exact context into the LLM.
...
4. **Boot the FAISS Vector Database & Server**
   ...
   python main.py
   ```
   *The script will load the BAAI embeddings and preexisting FAISS store into memory (~15 seconds).*
